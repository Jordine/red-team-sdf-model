"""CoT obfuscation SFT (Layer 3, stage A).

Fine-tune the adversarially-trained model on examples where the ``<think>``
block is internally-consistent but does not reveal the confidential fact.

Reference: Skaf et al. 2025 — https://arxiv.org/abs/2506.01926.

Expected input dataset: produced by ``build_cot_dataset.py``. Each row has a
``messages`` field with user + assistant turns; the assistant turn embeds the
``<think>...</think>`` block followed by the final response.

Training is a standard LoRA SFT; the only special handling is making sure the
chat template does not strip the think block and that the response mask
includes think-block tokens.

Usage::

    python obfuscation/cot_obfuscation.py \\
        --base-checkpoint data/checkpoints/adv_v1 \\
        --dataset data/transcripts/obfuscation_cot.jsonl \\
...
DEFAULT_OUTPUT = "data/checkpoints/obf_cot_v1"


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="LoRA SFT for CoT obfuscation (Layer 3 stage A).",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if adapter_path.exists() and (adapter_path / "adapter_config.json").exists():
        log.info("loading existing adapter from %s (trainable)", adapter_path)
        model = PeftModel.from_pretrained(base, str(adapter_path), is_trainable=True)
    else:
        log.info("no existing adapter found, initialising fresh LoRA")
    ...
    sft_config = SFTConfig(
        ...
        learning_rate=cot_cfg.get("learning_rate", 1e-4),
        num_train_epochs=cot_cfg.get("epochs", 3),
    ...
    log.info("saved CoT obfuscation checkpoint to %s", args.output)
    return 0
